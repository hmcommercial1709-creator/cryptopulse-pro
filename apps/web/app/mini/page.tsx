...
function Button(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  // Use the element's native onclick property instead of React's delegated
  // event system. This is the most reliable path inside Telegram Desktop WebViews.
...
const changeText = (change: number | null) => change == null ? '—' : `${change >= 0 ? '+' : ''}${change.toFixed(2)}%`;
const changeTone = (change: number | null) => change == null ? '#9aa4b2' : change >= 0 ? '#45d483' : '#ff6678';
...
export default function MiniTradingTerminal() {
  ...
  const [telegramAuthReady, setTelegramAuthReady] = useState(false);
  ...
  const applyHashRoute = useCallback((hash: string) => {
    ...
    else if (section.startsWith('asset_')) { setSelectedSymbol(section.replace(/^asset_/, '').toUpperCase()); setTab('trade'); }
  ...
  const navigateTab = useCallback((nextTab: Tab, symbol?: string) => {
    if (nextTab === 'home') {
      window.history.replaceState(null, '', '#markets');
    } else if (nextTab === 'intelligence') {
      window.history.replaceState(null, '', '#signals');
    } else if (nextTab === 'referral') {
      window.history.replaceState(null, '', '#referral');
    } else if (nextTab === 'pro') {
      window.history.replaceState(null, '', '#pro');
    } else if (nextTab === 'trade' && symbol) {
      window.history.replaceState(null, '', '#asset_' + encodeURIComponent(symbol.toLowerCase()));
    } else {
  ...
  useEffect(() => {
    ...
    // Telegram can finish wiring the WebView launch payload just after the
    // document becomes interactive. Poll briefly so a valid signed initData
    // cannot be missed by the first user action.
    let attempts = 0;
    const checkTelegramAuth = () => {
      const present = Boolean(getTelegramInitData());
      if (present) setTelegramAuthReady(true);
      attempts += 1;
      if (present || attempts >= 50) {
        setTelegramAuthChecked(true);
        return;
      }
      window.setTimeout(checkTelegramAuth, 100);
    };
    checkTelegramAuth();
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#070b14', color: '#f7f9fc', fontFamily: 'system-ui, sans-serif', padding: 16 }}>
      <section style={{ maxWidth: 620, margin: '0 auto' }}>
        ...
        {typeof window !== 'undefined' && telegramAuthChecked && !telegramAuthReady && !getTelegramInitData() && <div role="status" style={{ ...cardStyle, borderColor: '#6d5520', background: '#17130a', fontSize: 12, opacity: .9 }}>
          <strong>Telegram session not detected.</strong>
          <div style={{ marginTop: 5 }}>This launch did not provide signed Telegram initData, so personal actions cannot be authenticated. Re-open CryptoPulse from the bot's native Mini App entry (profile <b>Open App</b> or the chat menu button) rather than a normal browser URL.</div>
          <Button onClick={() => { try { getTelegramWebApp()?.openTelegramLink?.('https://t.me/CryptoPulseHubBot?startapp=auth'); } catch { window.location.href = 'https://t.me/CryptoPulseHubBot?startapp=auth'; } }} style={{ ...smallButtonStyle, marginTop: 10 }}>🔐 Re-open authenticated Mini App</Button>
        </div>}
                ...
                <div key={plan.code} style={{ ...cardStyle, marginBottom: 0 }}>
                  ...
                  <Button onClick={async () => {
                    ...
                    try {
                      ...
                      if (tg?.openInvoice) tg.openInvoice(body.invoiceUrl, (status) => {
                        if (status === 'paid') setShareMessage('Payment completed successfully.');
                        if (status === 'failed') setError('Telegram payment failed. Please try again.');
                        if (status === 'cancelled') setShareMessage('Payment cancelled.');
                      });
                      else if (tg?.openTelegramLink) tg.openTelegramLink(body.invoiceUrl);
        ...
        <nav aria-label="Primary navigation" style={{ position: 'sticky', bottom: 0, marginTop: 20, display: 'grid', gridTemplateColumns: 'repeat(8,1fr)', gap: 5, background: '#0d1320', padding: 8, borderRadius: 16, overflowX: 'auto' }}>{([['home','⌂'],['trade','⚡'],['intelligence','📊'],['watchlist','⭐'],['alerts','🔔'],['auto','🤖'],['portfolio','💼'],['referral','👥']] as const).map(([id,label]) => <Button key={id} onClick={() => navigateTab(id)} style={{ ...smallButtonStyle, opacity: tab === id ? 1 : .55, minWidth: 48 }}>{label}</Button>)}</nav>
      </section>
    </main>
  );
}
