...
export default function MiniTradingTerminal() {
  ...
  const shareSelected = async () => {
    ...
    try {
      const text = `⚡ CryptoPulse · ${selected.symbol}\nPrice ${money.format(selected.price)}\n24h ${selected.change24h >= 0 ? '+' : ''}${selected.change24h == null ? '—' : selected.change24h.toFixed(2)}%`;
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#070b14', color: '#f7f9fc', fontFamily: 'system-ui, sans-serif', padding: 16 }}>
      <section style={{ maxWidth: 620, margin: '0 auto' }}>
        ...
        {typeof window !== 'undefined' && !getTelegramWebApp()?.initData && <div role="status" style={{ ...cardStyle, borderColor: '#6d5520', background: '#17130a', fontSize: 12, opacity: .9 }}>Open this Mini App from Telegram to enable your personal watchlist, alerts, AI tasks, sharing and Stars checkout. Navigation remains available in a normal browser.</div>}
