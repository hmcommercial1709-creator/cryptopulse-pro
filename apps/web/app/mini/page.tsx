...
export default function MiniTradingTerminal() {
  ...
  const loadUserData = useCallback(async () => {
    ...
    try {
      ...
      const [watchResponse, alertResponse] = await Promise.all([
        fetch('/api/watchlist', { headers, cache: 'no-store' }),
        fetch('/api/alerts', { headers, cache: 'no-store' }),
      ]);
    ...
    } catch { /* keep market terminal usable if user data is temporarily unavailable */ }
  }, [authHeaders]);
  ...
  const toggleWatch = async (symbol: string) => {
    ...
    try {
      ...
      const response = await fetch(watched ? `/api/watchlist?symbol=${encodeURIComponent(symbol)}` : '/api/watchlist', {
        method: watched ? 'DELETE' : 'POST', headers: authHeaders(), body: watched ? undefined : JSON.stringify({ symbol }),
      });
      if (!response.ok) throw new Error(((await response.json()) as { error?: string }).error ?? 'Watchlist update failed.');
      await loadUserData();
      void track(watched ? 'watchlist_remove' : 'watchlist_add', { symbol });
    } catch (err) {
  ...
  const createAlert = async () => {
    ...
    if (!Number.isFinite(threshold)) { setError('Enter a valid alert threshold.'); return; }
    setBusy(true);
    try {
      ...
      setAlertThreshold('');
      await loadUserData();
      void track('alert_create', { symbol: alertSymbol, condition: alertCondition });
    } catch (err) {
  ...
  const removeAlert = async (id: string) => {
    setBusy(true);
    try {
      const response = await fetch(`/api/alerts?id=${encodeURIComponent(id)}`, { method: 'DELETE', headers: authHeaders() });
      if (!response.ok) throw new Error('Alert removal failed.');
      await loadUserData();
      void track('alert_remove', { id });
    } catch (err) { setError(err instanceof Error ? err.message : 'Alert removal failed.'); }
    finally { setBusy(false); }
  };
  ...
  const shareSelected = async () => {
    ...
    try {
      ...
      if (window.Telegram?.WebApp?.openTelegramLink) window.Telegram.WebApp.openTelegramLink(body.shareUrl);
      else window.open(body.shareUrl, '_blank', 'noopener,noreferrer');
      setShareMessage('Share card ready.');
      void track('first_share', { symbol: selected.symbol, cardType: 'market' });
    ...
    <main style={{ minHeight: '100vh', background: '#070b14', color: '#f7f9fc', fontFamily: 'system-ui, sans-serif', padding: 16 }}>
      <section style={{ maxWidth: 620, margin: '0 auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          ...
          <button onClick={() => void loadMarkets()} style={buttonStyle} disabled={loading}>{loading ? 'Loading…' : '↻ Refresh'}</button>
        </header>
        ...
        {tab === 'home' && <>
          {selected && <div style={cardStyle}>
            <div style={{ opacity: .65 }}>{selected.symbol} · CoinMarketCap</div>
            <div style={{ fontSize: 32, fontWeight: 800 }}>{money.format(selected.price)}</div>
            <div style={{ color: selected.change24h >= 0 ? '#45d483' : '#ff6678' }}>{selected.change24h >= 0 ? '+' : ''}{selected.change24h.toFixed(2)}% · 24h</div>
            <div style={{ opacity: .55, fontSize: 11, marginTop: 8 }}>Updated {updatedAt ? new Date(updatedAt).toLocaleTimeString() : '—'}{freshness !== null ? ` · ${freshness}s ago` : ''}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
              <button onClick={() => void toggleWatch(selected.symbol)} style={smallButtonStyle} disabled={busy}>{isWatched ? '★ In Watchlist' : '☆ Add Watchlist'}</button>
              <button onClick={() => void shareSelected()} style={smallButtonStyle} disabled={busy}>📤 Share Snapshot</button>
            </div>
          </div>}
          <h3>Live Market Scanner</h3>
          {loading && !markets.length && <div style={cardStyle}>Loading live market data…</div>}
        ...
        <nav style={{ position: 'sticky', bottom: 0, marginTop: 20, display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 5, background: '#0d1320', padding: 8, borderRadius: 16 }}>
          {([['home','⌂'],['trade','⚡'],['intelligence','📊'],['watchlist','⭐'],['alerts','🔔'],['auto','🤖'],['portfolio','💼']] as const).map(([id,label]) => <button key={id} onClick={() => setTab(id)} style={{ ...smallButtonStyle, opacity: tab === id ? 1 : .55 }}>{label}</button>)}
        </nav>
      </section>
    </main>
  );
