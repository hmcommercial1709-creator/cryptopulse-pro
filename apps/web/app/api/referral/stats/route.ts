...
export async function GET(request:NextRequest):Promise<Response>{
  try{
    ...
    const [dRows,nRows,cRows,lRows,commissionRows]=await Promise.all([
      ...
      supabaseSelect('cp_referral_commissions','referrer_user_id=eq.'+encodeURIComponent(userId)+'&status=eq.accrued&eligibility_status=eq.eligible&select=commission_stars,status,payment_stars,created_at&order=created_at.desc&limit=10000')
    ]);
