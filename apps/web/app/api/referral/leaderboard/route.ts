...
export async function GET(request:NextRequest):Promise<Response>{
  try{
    ...
    const rows=await supabaseSelect('cp_referral_leaderboard','select=rank,telegram_user_id,username,display_name,qualifying_paid_users,accrued_stars&order=rank.asc&limit=100') as Row[];
    const safe=rows.map((r)=>({
      ...
      isYou:Number(r.telegram_user_id)===Number(viewer.id)
    }));
